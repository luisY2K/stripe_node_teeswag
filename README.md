...
| `npm run create:subscription:add-streaming-to-delivery`      | **Case 6:** monthly Awesome Delivery (~2 months default); add stream with ladder stub → **90%×3 → 50%×3** (`free-trial` = €0 stub). Test clock defaults to **~7d** before delivery renewal (`stub short`); `stub long` ≈18d. Tenure: separate argv tokens `m N` after `--` (not `mN`). |
| `npm run create:subscription:add-streaming-stub-promo`       | **Spike (Case 6 variant):** stub = list streaming + coupon (**90%** default); **`free-trial`** → stub **100%** coupon then **100%×1 → 90%×2 → 50%×3**. Clock defaults **~7d** before renewal; `stub long` ≈18d. **`m N`** same as Case 6.                                              |
